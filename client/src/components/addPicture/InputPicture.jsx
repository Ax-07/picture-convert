...
export const InputPicture = ({ onInputChange, inputRef }) => {
  ...
  return (
    ...
    data-testid="inputPicture"
      className={`add-picture__wrapper ${dragging ? "add-picture__wrapper--dragging" : ""}`}
      onDragOver={dragOver}
    ...
    >
      <span className="add-picture__icon" role="icon">{icons.image}</span>
      <label htmlFor="picture" className="add-picture__wrapper add-picture__btn" role="button">{icons.plusCircle}</label>
      <input
          type="file"
          name="picture"
          id="picture"
        ...
        />
        <p className="add-picture__txt" role="txt">jpg, png : 15mo max</p>
    </div>
  );
};
